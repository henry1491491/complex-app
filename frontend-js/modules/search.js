// npm install dompurify => 避免 html 攻擊
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      <div class="list-group-item active"><strong>Search Results</strong> (${
        posts.length > 1 ? `${posts.length} items` : "1 item found"
      })</div>
     ...
     ${posts
      ...
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find ant results for that search.</p>`;
    }
    this.hideLoaderIcon();
